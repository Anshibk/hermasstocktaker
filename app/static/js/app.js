...
function normaliseEntryFromApi(entry){
  ...
  return {
    ...
    createdBy: (userObj && userObj.username) ? String(userObj.username) : (entry.user_name || ""),
...
function mapRoleToPayload(role){
  ...
  if(isAdminRole){
    ...
    payload.dashboard_scope = 'own';
    payload.add_item_scope = 'own';
    payload.raw_scope = 'own';
    payload.sfg_scope = 'own';
    payload.fg_scope = 'own';
    payload.entry_scope = 'own';
  }
  return payload;
}
...
function renameRoleDraft() {
  ...
  if (usersUi.roleDraft.meta?.isAdmin && usersUi.roleDraft.id) {
    toast("Default Admin role cannot be renamed.");
    return;
  }
  const current = usersUi.roleDraft.name || "";
  const name = prompt("New role name", current);
  if (!name) return;
  const trimmed = name.trim();
...
async function deleteRoleDraft() {
  if (!usersUi.roleDraft) return;
  if (usersUi.roleDraft.meta?.isAdmin) {
    toast("Default Admin role cannot be deleted.");
    return;
  }
...
function updateRoleField(path, value) {
  if (!usersUi.roleDraft) return;
  const adminLocked = !!(usersUi.roleDraft.meta?.isAdmin && usersUi.roleDraft.id);
  if (adminLocked) {
...
function renderUsersPage() {
  ...
  const roleListHtml = roleList.length
    ? roleList
        .map((role) => {
          ...
          const defaultLabel = role.meta?.isAdmin ? '<span class="hint">Default</span>' : '';
  ...
  const isAdminRole = !!roleDraft?.meta?.isAdmin;
  const adminRoleLocked = !!(isAdminRole && roleDraft?.id);
  const canToggleRolePermissions = canManageRoles && !adminRoleLocked;
  const tabConfig = [
  ...
  if (isAdminRole) {
    tabConfig.push(["broadcast", "Broadcast Control"]);
  }
  ...
  const deleteLocked = !canManageRoles || !roleDraft || roleDraft.meta?.isAdmin;
  ...
  let exportPanel = '';
  let actionsPanel = '';
  let broadcastPanelMarkup = '';
  if (roleDraft) {
    visibilityPanel = `
      <div class="panel ${roleTab === 'visibility' ? 'active' : ''}" data-panel="visibility">
        <div class="card p-4${canToggleRolePermissions ? '' : ' is-disabled'}">
          <div class="font-semibold mb-1">1. Page Visibility</div>
          <div class="hint mb-3">(Page Access configuration)</div>
          <div class="segAuto">
            <label class="flex items-center gap-3"><input data-k="pv.dashboard" type="checkbox" class="h-4 w-4"${roleDraft.pv?.dashboard ? ' checked' : ''}${checkboxDisabledAttr}><span>Dashboard</span></label>
            <label class="flex items-center gap-3"><input data-k="pv.addItem" type="checkbox" class="h-4 w-4"${roleDraft.pv?.addItem ? ' checked' : ''}${checkboxDisabledAttr}><span>Master Data</span></label>
            <label class="flex items-center gap-3"><input data-k="pv.raw" type="checkbox" class="h-4 w-4"${roleDraft.pv?.raw ? ' checked' : ''}${checkboxDisabledAttr}><span>Raw Materials</span></label>
            <label class="flex items-center gap-3"><input data-k="pv.sfg" type="checkbox" class="h-4 w-4"${roleDraft.pv?.sfg ? ' checked' : ''}${checkboxDisabledAttr}><span>Semi Finished</span></label>
            <label class="flex items-center gap-3"><input data-k="pv.fg" type="checkbox" class="h-4 w-4"${roleDraft.pv?.fg ? ' checked' : ''}${checkboxDisabledAttr}><span>Finished Goods</span></label>
            <label class="flex items-center gap-3"><input data-k="pv.manageData" type="checkbox" class="h-4 w-4"${roleDraft.pv?.manageData ? ' checked' : ''}${checkboxDisabledAttr}><span>Manage Data</span></label>
            <label class="flex items-center gap-3"><input data-k="pv.users" type="checkbox" class="h-4 w-4"${roleDraft.pv?.users ? ' checked' : ''}${checkboxDisabledAttr}><span>Users</span></label>
          </div>
          <div class="section-divider"></div>
          <div class="hint">Toggle which pages the role can access.</div>
        </div>
      </div>`;
    exportPanel = `
      <div class="panel ${roleTab === 'export' ? 'active' : ''}" data-panel="export">
        <div class="card p-4${canToggleRolePermissions ? '' : ' is-disabled'}">
          <div class="font-semibold mb-1">2. Export/Import</div>
          <div class="grid gap-3">
            <label class="flex items-center gap-3">
              <input data-k="ei.exportData" type="checkbox" class="h-4 w-4"${roleDraft.ei?.exportData ? ' checked' : ''}${checkboxDisabledAttr}>
              <span>Export Data</span>
            </label>
            <div id="wrapExportWithMaster" class="${wrapExportWithMasterClass}">
              <label class="flex items-center gap-3">
                <input data-k="ei.exportWithMaster" type="checkbox" class="h-4 w-4"${roleDraft.ei?.exportWithMaster ? ' checked' : ''}${exportSubDisabledAttr}>
                <span>Export with master items</span>
              </label>
              <p class="hint pl-7 mt-1">Exports all items from master data, even those with no entries in the selected period.</p>
            </div>
            <div id="wrapExportValuated" class="${wrapExportValuatedClass}">
              <label class="flex items-center gap-3">
                <input data-k="ei.exportValuated" type="checkbox" class="h-4 w-4"${roleDraft.ei?.exportValuated ? ' checked' : ''}${exportSubDisabledAttr}>
                <span>Export valuated items</span>
              </label>
              <p class="hint pl-7 mt-1">Exports only items that have at least one entry in the selected period.</p>
            </div>
            <div class="section-divider"></div>
            <label class="flex items-center gap-3">
              <input data-k="ei.importData" type="checkbox" class="h-4 w-4"${roleDraft.ei?.importData ? ' checked' : ''}${checkboxDisabledAttr}>
    ...
    actionsPanel = `
      <div class="panel ${roleTab === 'actions' ? 'active' : ''}" data-panel="actions">
        <div class="card p-4${canToggleRolePermissions ? '' : ' is-disabled'}">
          <div class="font-semibold mb-1">3. Actions</div>
          <div class="hint mb-3">Configure which interactions are available in each module.</div>
          ${renderActionGroup('add', 'Add entry:')}
          <div class="section-divider"></div>
          ${renderActionGroup('bulk', 'Bulk delete:')}
          <div class="section-divider"></div>
          ${renderActionGroup('edit', 'Edits:')}
        </div>
      </div>`;
    if (isAdminRole) {
      const broadcastSections = [
        ["dashboard", "Dashboard"],
        ["addItem", "Master Data"],
        ["raw", "Raw Materials"],
        ["sfg", "Semi Finished"],
      ...
      const broadcastHtml = broadcastSections
        .map(([key, label]) => {
          const checked = (roleDraft.sc?.[key] || "OWN") === "ORG";
          return `
            <div>
              <div class="font-medium text-slate-700">${H(label)}</div>
              <label class="mt-2 inline-flex items-center gap-2 text-sm text-slate-600">
                <input type="checkbox" data-broadcast="${H(key)}" class="h-4 w-4"${checked ? ' checked' : ''}${broadcastDisabledAttr}>
                <span>Share data</span>
              </label>
            </div>`;
        })
        .join("");
      broadcastPanelMarkup = `
        <div class="panel ${roleTab === 'broadcast' ? 'active' : ''}" data-panel="broadcast">
          <div class="card p-4${canManageRoles ? '' : ' is-disabled'}">
            <div class="font-semibold mb-1">4. Broadcast Control</div>
            <div class="hint mb-3">Allow everyone to see the admin's complete view for each module.</div>
  ...
  if (broadcastPanelMarkup) {
    panelPieces.push(broadcastPanelMarkup);
  }
  ...
  const roleEditorContent = `
    <div class="flex items-center justify-between mb-3">
      <div class="flex items-center gap-2">
        <span id="roleNameChip" class="role-pill">Role: ${H(roleNameLabel)}</span>
        ${adminRoleLocked ? '<span class="role-pill muted">Locked</span>' : ''}
        <span id="dirtyChip" class="${dirtyChipClass}">Unsaved</span>
      </div>
      <div class="tabs" id="roleTabs">${tabsHtml}</div>
    </div>
    <div class="section-divider"></div>
    ${rolePanels}
    <div class="pt-3 border-t border-slate-200 mt-3 flex items-center justify-end gap-2">
      <button id="btnDiscardRole" class="btn">Discard</button>
  ...
  page.innerHTML = `
    ...
    <div id="rolesModal" class="modal${usersUi.rolesModalOpen ? ' show' : ''}">
      <div class="mask" data-close></div>
      <div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[min(96vw,1100px)] card overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
          <div class="font-semibold text-slate-800">Manage Roles</div>
          <div class="flex items-center gap-2">
            <button class="btn" data-close>Close</button>
          </div>
        </div>
        <div class="p-4 two-col">
          <aside class="card p-4 h-[68vh] overflow-y-auto" data-scroll-key="roleListScroll">
            <div class="flex items-center justify-between mb-3">
              <div class="font-semibold text-slate-800">Roles</div>
              <button id="btnNewRole" class="${newRoleBtnClass}"${newRoleBtnAttr}>+ Add</button>
            </div>
            <div id="rolesList" class="space-y-2">${roleListHtml}</div>
            <div class="my-4 h-px bg-slate-200"></div>
            <div class="space-y-2">
              <button id="btnRenameRole" class="${renameBtnClass}"${renameBtnAttr}>Rename</button>
              <button id="btnDuplicateRole" class="${duplicateBtnClass}"${duplicateBtnAttr}>Duplicate</button>
              <button id="btnDeleteRole" class="${deleteBtnClass}"${deleteBtnAttr}>Delete</button>
            </div>
          </aside>
          <section class="card p-4 h-[68vh] overflow-hidden flex flex-col">
            ${roleEditorContent}
          </section>
        </div>
      </div>
...
function renderEntryPage(type){
  ...
  const paint=()=>{
    const isAdminUser = String(state.currentUser?.role || "").toLowerCase() === "admin";
    ...
    keys.forEach(k=>{
      ...
      arr.forEach((r,i)=>{
        ...
        const canManageEntry = isAdminUser || r.userId === state.currentUserId;
        if (canManageEntry) {
...
function toast(msg){
  ...
  clearTimeout(toastT); toastT=setTimeout(()=>t.classList.add("hidden"),2000);
}
